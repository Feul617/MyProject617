...
class Node:
    ...
    def add_children(self, *children): # *은 함수 호출에 있어서 가변인자(개수가 달라지는)를 받기 위해 사용
        for child in children:
            self.children.append(child)


class SelectorNode(Node): #자식 노드를 왼쪽부터 오른쪽으로 차례대로 실행 / 그 중에 하나라도 성공하면 성공
    def __init__(self, name):
        self.children = []
        self.name = name
        self.prev_running_pos = 0 # 앞선 실행에서, RUNNING으로 리턴한 자식 노드 위치를 저장 -> 위치를 바로 찾기 위해서?
    def run(self):
...
class SequenceNode(Node): # 하위 노드들이 다 성공해야지 성공
    def __init__(self, name):
        self.children = []
        self.name = name
        self.prev_running_pos = 0
